...
得到两个数组的并集, 两个数组的元素为数值或字符串。 例：

```javascript
var numOne = [0, 2, 4, 6, 8, 8];
var numTwo = [1, 2, 3, 4, 5, 6];
var duplicatedValues = Array.from(new Set([...numOne, ...numTwo]))
console.log(duplicatedValues);
// returns [0, 2, 4, 6, 8, 1, 3, 5]
```
...
[你应该知道的13个有用的JavaScript数组技巧](https://mp.weixin.qq.com/s/1bfCI3YgsfwMYmVwknfNjA)
